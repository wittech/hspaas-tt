...
public class ParameterUtil {
     ...
     */
    public static JSONObject transform(Map<String, String[]> properties) throws UnsupportedEncodingException {
        ...
        for (String key : properties.keySet()) {
            ...
            dto.put(key, java.net.URLDecoder.decode(properties.get(key)[0], ENCODING));
        }
     ...
     * @throws UnsupportedEncodingException
     */
    public static JSONObject transformInGetMode(String qureyString) throws UnsupportedEncodingException {
        if (StringUtils.isEmpty(qureyString)) {
            return null;
        }

        String queryStr = java.net.URLDecoder.decode(qureyString, ENCODING);
    ...
    public static JSONObject transform(InputStream is, int contentLen, String encoding) {
        ...
        if (contentLen > 0) {
            ...
            try {
                ...
                // 自定义参数包名称，后台exchanger 反解析， 默认字符编码UTF-8
                jsonObject.put(ParameterFilterContext.PARAMETER_NAME_IN_STREAM, new String(message, ENCODING));
    ...
    public static JSONObject transform(InputStream is, int contentLen) {
        return transform(is, contentLen, ENCODING);
    }
