package com.huashi.exchanger.resolver.http.custom.itissm;
...
//			{\"account\":\"HSKJYX\",\"GetReport\":\"2115238738241199173,18368031231,DELIVRD|2115238715962088136,18368031231,DELIVRD|\"}
			...
			/**
			 * 数据格式：批次,号码,时间,状态|
            *  批次A,号码A,时间A,状态A|批次B,号码B,时间B,状态B |批次C,号码C,时间C,状态C| ……
            *  示例：
            *    2114355899380234221,13900000000,2014/06/10 15:34:11,DELIVRD|
            *    2114355899380234221,13900000001,2014/06/10 15:34:11,DELIVRD|
            *    2114355899380234221,13900000002,2014/06/10 15:34:11,UNDELIVRD |
			 */
...
//	          {"GetMo":"15868193450|,|￥ﾥﾽ￧ﾚﾄ￯ﾼﾌ￦ﾈﾑ￧ﾟﾥ￩ﾁﾓ￤ﾺﾆ|,|2018/4/16 11:14:13|,|106914010526|;|15868193450|,|￥ﾗﾯ￥ﾗﾯ￯ﾼﾌ￥ﾏﾯ￤ﾻﾥ|,|2018/4/16 11:14:13|,|106914010526|;|","account":"HSKJYX"}
			...
			/** 
			 * 数据格式：
		     *   A号码|,|A回复内容|,|A回复时间|;| B号码|,|B回复内容|,|B回复时间……
		     *   1) 每个号码及其回复、回复时间称为一组回复，号码、回复内容和回复时间之间用“|,|”分隔；
		     *   2) 每组回复之间，用“|;|”分隔；
		     *   3) 没有回复时，返回空字符串。
			 */
	...
	用户名密码错误或者用户无效
	-5
	签名不正确（格式为:XXXX【签名内容】）注意，短信内容最后一个字符必须是】
	-6:keyWords
	含有关键字keyWords （keyWords为敏感内容，如： -6:促销）
	-7
	余额不足
	-8
	没有可用通道，或不在时间范围内
	...
	发送号码一次不能超过1000个
	-10
	号码数量大于允许上限（不设置上限时，不可超过1000）
	-11
	号码数量小于允许下限
	-12
	模板不匹配
	-13
	Invalid Ip ip绑定用户，未绑定该ip
	-14
	用户黑名单
	-15
	系统黑名单
	...
	号码格式错误
	-17
	无效号码（格式正常，可不是正确的电话号码,如12345456765）
	-18
	没有设置用户的固定下发扩展号，不能自定义扩展
	-19
	强制模板通道，不能使用个性化接口
	-20
	包含非法字符
