 ...
 */
@Component
public class SmsWaitMoPushListener extends BaseListener implements MessageListener {
    ...
    private void doTranslateHttpReport(SmsMoMessageReceive report) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("处理结果：{}, 尝试次数 : {}, 异常信息: {}", retryResponse.getResult(),
                             retryResponse.getAttemptTimes(), retryResponse.getLastThrowable().getMessage());
            }
    ...
    @Override
    @JmsListener(destination = ActiveMqConstant.MQ_SMS_MO_WAIT_PUSH)
    ...
    private void doPushPersistence(SmsMoMessageReceive report, String content, RetryResponse retryResponse,
        ...
        stringRedisTemplate.opsForList().rightPush(SmsRedisConstant.RED_DB_MESSAGE_MO_PUSH_LIST,
