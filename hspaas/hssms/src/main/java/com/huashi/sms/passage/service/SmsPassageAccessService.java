...
public class SmsPassageAccessService implements ISmsPassageAccessService {
	...
	public boolean save(SmsPassageAccess access) {
		...
		try {
			stringRedisTemplate.opsForHash().put(getMainKey(access.getUserId(), access.getCallType()),
					getAssistKey(access.getRouteType(), access.getCmcp(), access.getProvinceCode()),
					JSON.toJSONString(access));

			// 如果通道调用类型为 自取，需要同步到 监管中心
			if (PassageCallType.STATUS_RECEIPT_WITH_SELF_GET.getCode() == access
					.getCallType()
					|| PassageCallType.SMS_MO_REPORT_WITH_SELF_GET.getCode() == access
							.getCallType()) {
	...
	public boolean reload() {
		// 加载所有待发送可用通道信息
		List<SmsPassageAccess> list = smsPassageAccessMapper
				.selectByType(PassageCallType.DATA_SEND.getCode());
	...
	public boolean update(SmsPassageAccess access) {
		try {
			...
			logger.warn(
					"Redis 用户可用通道信息更新失败, userId : {}, cmcp : {}, routeType :{}",
					access.getUserId(), access.getCmcp(),
	...
	@Override
	public BossPaginationVo<SmsPassageAccess> findPage(int pageNum,
		...
		if (list != null && list.size() > 0) {
			for (SmsPassageAccess p : list) {
				if (passageMap.containsKey(p.getPassageId())) {
					...
					SmsPassage smsPassage = smsPassageMapper
							.selectByPrimaryKey(p.getPassageId());
					if (smsPassage != null) {
				...
				if (provinceMap.containsKey(p.getProvinceCode())) {
					...
					p.setProvinceName(province == null ? "未知" : province
							.getName());
	...
	public boolean updateByModifyUser(int userId) {
		try {
			...
			UserPassage userPassage = userPassageService.getByUserIdAndType(
					userId, PassageTemplateType.SMS.getValue());
			if (userPassage == null) {
			...
			// 根据通道组ID查询通道组详细信息
			List<SmsPassageGroupDetail> detailList = smsPassageGroupDetailMapper
					.findPassageByGroupId(userPassage.getPassageGroupId());
			if (CollectionUtils.isEmpty(detailList)) {
				logger.error("通道组ID：{} 查不到相关短信通道集合数据",
			...
			for (SmsPassageGroupDetail detail : detailList) {
				...
				// 如果通道状态为不可用，直接忽略
				if (detail.getSmsPassage() == null
						|| detail.getSmsPassage().getStatus() == null
						|| PassageStatus.ACTIVE.getValue() != detail
								.getSmsPassage().getStatus())
					continue;

				if (isDone(distinctAccessKeys, detail))
					continue;

				replacePassageValue2Access(detail, userId,
						userPassage.getPassageGroupId());
			}
	 ...
	 * @return
	 */
	private static String getAccessCutRepeatKey(Integer routeType,
			Integer cmcp, Integer provinceCode) {
		return String.format("%d:%d:%d", routeType, cmcp, provinceCode);
	}
	 ...
	 * @return
	 */
	private boolean isDone(Set<String> distinctAccessKeys,
			SmsPassageGroupDetail detail) {
		...
		return !distinctAccessKeys.contains(getAccessCutRepeatKey(
				detail.getRouteType(), detail.getCmcp(),
	 ...
	 *            通道组ID
	 */
	private void replacePassageValue2Access(SmsPassageGroupDetail detail,
			Integer userId, Integer passageGroupId) {
		SmsPassage smsPassage = detail.getSmsPassage();

		try {
			List<SmsPassageParameter> parameterList = smsPassageParameterMapper
					.findByPassageId(detail.getPassageId());
	...
	public boolean updateByModifyPassageGroup(int passageGroupId) {
		try {
			List<UserPassage> userPassageList = userPassageService
					.getPassageGroupListByGroupId(passageGroupId);
	...
	public boolean updateByModifyPassage(int passageId) {
		try {
			List<Integer> groupIdList = smsPassageGroupDetailMapper
					.getGroupIdByPassageId(passageId);
	...
	public SmsPassageAccess get(int id) {
		...
		if (access != null) {
			...
			// 设置路由类型名称
			access.setRouteTypeText(RouteType.parse(access.getRouteType())
					.getName());
	...
	public List<SmsPassageAccess> findPassageBalace() {
		return smsPassageAccessMapper
				.selectByType(PassageCallType.PASSAGE_BALANCE_GET.getCode());
	}

	@Override
	public SmsPassageAccess getByType(PassageCallType callType,
			String passageCode) {
		return smsPassageAccessMapper.getByTypeAndUrl(callType.getCode(),
				passageCode);
	}
	...
	public boolean deletePassageAccess(Integer passageId) {
		try {
			...
			for (SmsPassageAccess access : list) {
				stringRedisTemplate.opsForHash().delete(getMainKey(access.getUserId(), access.getCallType()),
								getAssistKey(access.getRouteType(), access.getCmcp(), access.getProvinceCode()));

				// 如果通道调用类型为 自取，需要同步到 监管中心（停止轮训扫描）
				if (PassageCallType.STATUS_RECEIPT_WITH_SELF_GET.getCode() == access
						.getCallType()
						|| PassageCallType.SMS_MO_REPORT_WITH_SELF_GET
								.getCode() == access.getCallType()) {
	...
	public boolean updateAccessStatus(Integer passageId, Integer status) {
		if (passageId == null || status == null) {
			logger.error("数据异常，更改可用通道状态失败，passageId : {}, status : {}",
					passageId, status);
			return false;
		}
		...
		try {
			...
			int result = smsPassageAccessMapper.updateStatusByPassageId(
					passageId, status);
			if (result == 0)
			...
			logger.error("修改可用通道状态失败，通道ID: {}， 状态：{} 失败信息", passageId, status,
					e);
			return false;
		}
	...
	public Map<String, SmsPassageAccess> getByUserId(int userId) {
		...
		try {
			Map<Object, Object> entries = stringRedisTemplate.opsForHash().entries(getMainKey(userId, PassageCallType.DATA_SEND.getCode()));
			...
			List<SmsPassageAccess> list = smsPassageAccessMapper.selectByUserIdAndCallType(userId, PassageCallType.DATA_SEND.getCode());
			if(CollectionUtils.isEmpty(list))
				return null;
	...
	@Override
	public SmsPassageAccess get(int userId, int route, int cmcp,
			int provinceCode) {
		try {
			Object object = stringRedisTemplate.opsForHash().get(getMainKey(userId, PassageCallType.DATA_SEND.getCode()),
					getAssistKey(route, cmcp , provinceCode));
			if (object != null)
				return JSON.parseObject(object.toString(), SmsPassageAccess.class);

		} catch (Exception e) {
			logger.warn("Redis 用户可用通道信息查询失败, userId : {}, cmcp : {}", userId,
					cmcp, e);
		}
		...
		try {
			...
			stringRedisTemplate.opsForHash().put(getMainKey(access.getUserId(), access.getCmcp()),
					getAssistKey(access.getRouteType(),access.getCallType(), access.getProvinceCode()),
					JSON.toJSONString(access));
		} catch (Exception e) {
			logger.warn("Redis 用户可用通道信息添加失败, userId : {}, cmcp : {}", userId,
					cmcp, e);
		}
		return access;
	}
