        ...
        render: function() {
            var that = this,
                ...
                _tpl = [
                    ...
                    '<dd>',
                    '<a href="javascript:;" data-title="{{it.title}}" data-icon="{{it.icon}}" data-id="{{it.id}}" >',
                    '{{# if (it.icon.indexOf("fa-") !== -1) { }}',
                    '<i class="fa {{it.icon}}" aria-hidden="true"></i>',
                    '{{# } else { }}',
                    '<i class="layui-icon">{{it.icon}}</i>',
                    '{{# } }}',
                    '<span> {{it.title}}</span>',
                    '</a>',
                    '</dd>',
            ...
            var tIndex = setInterval(function() {
                ...
                laytpl(_tpl.join('')).render(_data, function(html) {
                    ...
                    typeof _config.onClicked === 'function' && _elem.find('.layui-nav-child dd').off('click').on('click', function() {
                        var _a = $(this).children('a'),
                            id = _a.data('id');
                        _config.onClicked(id);
                    });
